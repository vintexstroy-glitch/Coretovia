/**
 * РЕДЪТ · родовите команди за всяка таблица с редове (ADR-005).
 *
 * Поправка на клетка (в клетката, не бутон), изключване и връщане на ред (десен
 * бутон) са ЕДНИ и същи за Имоти, Обекти, Бизнеси и Задачи — колоните са данни, а
 * предусловията четат Модела: клетките са живи (номенклатурният номер съществува и
 * не е спрян, връзката сочи жив ред от позволена таблица, Видът принадлежи на
 * Категорията), Обект не взима категория със своя таблица, номерът (кортежът) е
 * свободен, смяната на Категория иска нов Вид, задължителна колона не се изпразва,
 * `do` не е преди `ot`. Прозорците на командите се ИЗВЕЖДАТ от Модела, не се преписват.
 *
 * Тук е и родовият строител на команда „нов ред" (`komandaZaNovRed`), който Имоти и
 * Управление ползват с неговите имена на бутоните.
 */

import type { Kletka, Kletki } from '../../model/kletka.js';
import { sashtnost, type Vid } from '../../model/klyuchove.js';
...
/** прозорците с таблици · изведени от Модела, не преписани */
const PROZORTSI_S_TABLITSI = [...new Set(TABLITSI.map((t) => t.prozorets))];
...
function proveriKletkite(
  ...
  for (const [klyuch, kl] of Object.entries(kletki)) {
    ...
    if (kol.vid === 'vrazka' && 'tekst' in kl) {
      const kamT = tablitsaNaVrazkata(k.model, kol, kl.tekst);
      const kam = kamT === undefined ? undefined : k.ogledalo.tablitsi.get(kamT.klyuch);
      const i = kam?.indeks.get(kl.tekst);
      if (kamT === undefined || kam === undefined || i === undefined)
        n.push(`Няма ред „${kl.tekst}" в „${(kol.vrazka ?? []).join('" · „')}".`);
      else if (kam.izklyuchen[i] === 1)
        n.push(
          `Редът „${imeNaReda(k.ogledalo, kamT.klyuch, kl.tekst)}" е изключен — върни го първо.`,
        );
    }
  }
  if (n.length > 0) return n;
...
/**
 * Заместителят `@predlozhenie:N:‹вид›` във връзка → id-то на реда, който предложение N
 * ражда: същото извеждане като в `dryRun` (`idNaRed` от ключа на действието), затова
 * Обектът под нов Имот от същата Книга сочи точно него. Без вид (стар запис) — първата
 * позволена таблица на връзката.
 */
function zamestiPredlozheniyata(t: Tablitsa, kletki: Kletki, idNa: IdNaPredlozhenie): Kletki {
  ...
  for (const [klyuch, v] of Object.entries(kletki)) {
    ...
    if (
      ...
      const [n, vid] = v.tekst.slice(PREDLOZHENIE.length).split(':');
      const sashtnost =
        (vid as Vid | undefined) ?? tablitsata(MODEL, kol.vrazka[0] ?? '').sashtnost;
      rez[klyuch] = { tekst: idNaRed(sashtnost, idNa(Number(n))) };
    } else rez[klyuch] = v;
  }
  return rez;
}
...
function proveriDatite(t: Tablitsa, sled: Red): string[] {
  ...
  for (const sl of t.slyati ?? []) {
    ...
    const doo = sled[sl.opashka];
    const kolOt = kolonaNa(t, sl.kolona);
    if (kolOt?.vid !== 'data') continue;
    if (
      ot !== undefined &&
      doo !== undefined &&
      'tekst' in ot &&
      ...
      n.push(`Краят (${doo.tekst}) е преди началото (${ot.tekst}).`);
  }
  return n;
}
...
export const redPopraviKletka = Object.freeze(popraviKletka);
...
function zhiviDetsa(
  ...
  for (const t of k.model.tablitsi.values()) {
    ...
    const broy = zhiviteRedove(tv).filter((i) =>
      vrazki.some((c) => {
        const kl = kletkaNa(tv, i, c.klyuch);
