...
/**
 * МОДЕЛЪТ НА ЕКРАНА · неговите „Отвори" и „Запази" (A14 · B14 на Управление).
 *
 ...
 *
 * МОДЕЛ е ИМЕНУВАНА снимка на това КАК се гледа един прозорец — такт, период,
 * филтър, скрити, избраните сметки. Не е данни: данните са в Журнала и не се
 * пипат от него. Затова моделът е ЕДИН товар с непрозрачна за домейна карта:
 * екранът знае какво значат ключовете, Журналът пази само че са записани.
 */
export interface PayloadModelZapisan {
  readonly prozorets: string;
  /** името, което човекът е дал · то е и ключът на модела в този прозорец */
  readonly ime: string;
  /** снимката на екранната памет · ключ → стойност, каквато екранът я пази */
  readonly snimka: Readonly<Record<string, unknown>>;
  /** махнат модел · остава в Журнала, спира да се предлага */
