...
describe('четене на сума, написана от човек', () => {
  it('приема запетая, точка и разредка', () => {
    expect(otSuma('1150,50')).toBe(1150_50);
    expect(otSuma('1150.50')).toBe(1150_50);
    expect(otSuma('1 150,50')).toBe(1150_50);
    expect(otSuma('1150')).toBe(1150_00);
    expect(otSuma('0,05')).toBe(5);
    expect(otSuma('-1150,50')).toBe(-1150_50);
  });

  it('допълва липсващата втора цифра', () => {
    expect(otSuma('1150,5')).toBe(1150_50);
  });

  it('отказва всичко, което не е сума', () => {
    for (const losho of ['', 'абв', '1150,505', '1150,', '1.2.3', '12,5лв', '1e3']) {
      expect(() => otSuma(losho)).toThrow(GreshkaPari);
    }
  });
});
