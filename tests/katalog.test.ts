...
describe('каталогът', () => {
  it('двайсет и пет команди · уникални ключове · `prozorets.glagol`', () => {
    expect(KATALOG).toHaveLength(25);
    const klyuchove = KATALOG.map((k) => k.klyuch);
    expect(new Set(klyuchove).size).toBe(25);
