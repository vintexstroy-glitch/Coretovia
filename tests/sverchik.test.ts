...
describe('неговата Книга · мострата срещу празно Огледало', () => {
  it('61 предложения · петте Имота · деветте задачи · шестимата му хора · единайсетте продажби', async () => {
    ...
    expect(vidove.filter((v) => v === 'nov-red')).toHaveLength(61);
    ...
    // двете му таблици с продажби · шест реда в първата сграда, пет във втората
    expect(poTablitsa('prodazhbi')).toHaveLength(6);
    expect(poTablitsa('prodazhbi2')).toHaveLength(5);
    expect(
  ...
  it('приети до едно · Огледалото носи Книгата му · после е неподвижна точка · същите ключове = повторено', async () => {
    ...
    expect(vtori.povtoreni).toBe(61);
