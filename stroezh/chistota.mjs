 ...
 *       и `new Function` вън от `app/reshetka/shablon.ts`. Праг НУЛА (резен 6м);
 *   8б · ДУБЛИРАНО ПО СТРУКТУРА · същата ФОРМА под различни имена. Обход 8 не
 *       вижда нищо, което се различава по едно име — а точно такова е скъпото
 *       дублиране тук (резен 6з).
 *
...
const kod = [...vsichkiFaylove(join(KOREN, 'src')), ...vsichkiFaylove(join(KOREN, 'app'))];
...
const PRAZNI = [
  ...
  // ЗАКРЪГЛЯНЕТО НА ПАРИ ИМА ЕДИН ДОМ (резен 6и · правило 3).
  //
  // `deliZakragleno` живее в `yadro/pari.ts` и дълго имаше ЕДИН викащ, докато
  // същата операция беше пренаписана на четири места с `Math.round` — и точно
  // там, където числото застава пред него (Продажби · Калкулаторът). Двете НЕ
  // са едно: `Math.round(-2.5)` е -2, а половинката тук се закръгля ОТ нулата.
  // Сторното е обърнат знак, тъй че разликата излиза на сверка, която не затваря.
  [
    /Math\.round\s*\([^\n]*_st\b/g,
    'Math.round върху пари · домът на закръглянето е `deliZakragleno` (правило 3)',
  ],
];
...
// ── 8б · ДУБЛИРАНО ПО СТРУКТУРА · еднаква ФОРМА, различни имена ───────────

/**
 * ОБХОД 8 ЛОВИ ДОСЛОВНОТО. ТОЗИ ЛОВИ ФОРМАТА.
 *
 * Обход 8 иска ПЕТ дословно еднакви реда. Той брои нула — и числото е вярно, —
 * но мери малко: всяко дублиране, което се различава по ЕДНО име, му е
 * невидимо. А точно такова е скъпото дублиране в този проект: два записа от
 * форма с различен префикс на белега, две фабрики за команда-близнак, осем
 * писача на лист, които повтарят „инструкции · лента · глава · данни".
 *
 * Затова тук редовете се НОРМАЛИЗИРАТ, преди да се сравнят: всяко име става
 * `N`, всеки низ — `S`, всяко число — `0`, интервалите падат. Остава СКЕЛЕТЪТ.
 *
 * ═══ ЗАЩО ПЕТ РЕДА И КЛЮЧ ≥ 80 ═══
 *
 * Мерено с ръка върху цялото хранилище: при четири реда и по-къс ключ броят
 * скача на двайсет и повече, и почти всичко е шум (`if` · `return` · извикване).
 * Праг, който вика при всяко второ извикване, се изключва от човек в първата
 * седмица — и тогава обходът е по-лош от липсващ (ADR-051).
 *
 * При пет реда и ключ от поне осемдесет знака остават ТРИ места, и трите са
 * истински. Числото е мерено, не избрано наслуки.
 *
 * ═══ ПРОЗОРЦИТЕ СЕ СЛИВАТ ═══
 *
 * Плъзгащият прозорец обявява един и същ блок по веднъж за всяко свое
 * отместване — три находки за едно място. Затова съседните се сливат: доклад,
 * който брои едно нещо три пъти, кара човек да не му вярва.
 */
const NORMALIZIRAY = (red) =>
  red
    .trim()
    .replace(/'[^']*'|"[^"]*"|`[^`]*`/g, 'S')
...
const REDA_V_PROZORETSA = 5;
const NAY_KASIYAT_KLYUCH = 80;

const skeleti = new Map();
const strukturni = [];
for (const f of kod) {
  const redove = chist
    .get(f)
  ...
  for (let i = 0; i + REDA_V_PROZORETSA <= redove.length; i += 1) {
    const par = redove.slice(i, i + REDA_V_PROZORETSA);
    const klyuch = par.map(([r]) => NORMALIZIRAY(r)).join('|');
    if (klyuch.length < NAY_KASIYAT_KLYUCH) continue;
    const tuk = { kade: f, red: par[0][1] };
    const veche = skeleti.get(klyuch);
    if (veche === undefined) {
      skeleti.set(klyuch, tuk);
      continue;
    }
    strukturni.push({ tuk, veche });
  }
}

// СЛИВАНЕ на съседните прозорци · един блок = една находка
let posledna = null;
for (const { tuk, veche } of strukturni) {
  if (
    posledna !== null &&
    posledna.kade === tuk.kade &&
    tuk.red - posledna.red <= REDA_V_PROZORETSA
  ) {
    posledna = tuk;
    continue;
  }
  ...
  nahodka(
    ...
    tuk.kade,
    tuk.red,
    `същата ФОРМА като ${izravni(relative(KOREN, veche.kade))}:${veche.red} · различни имена`,
  );
}
vidyal('8б · дублирано по структура', kod.length);

// ── 9 · ПЪТ ДО HTML ИЗВЪН ВРАТАТА ─────────────────────────────────────────

/**
 * ЕДНА ВРАТА, И ТЯ Е ЗАПЕЧАТАНА (резен 6м · ADR-019).
 *
 * Дотук екранът се пазеше с НАВИК: една екранираща функция, викана на 185
 * места, и всяко от тях трябваше да бъде написано. Едно забравено — и чуждо име
 * от негов `.xlsx` става изпълним код, а един изпълним ред тук чете ЦЕЛИЯ
 * Журнал и пише през Портата.
 *
 * Сега пътят е ЕДИН: `app/reshetka/shablon.ts`. Този обход пази да остане един.
 *
 * ═══ ЗАЩО ОБХОД, СЛЕД КАТО БРАУЗЪРЪТ ВЕЧЕ ОТКАЗВА ═══
 *
 * Trusted Types отказва при ИЗПЪЛНЕНИЕ, в браузър, който я поддържа. Обходът
 * отказва при СГЛОБЯВАНЕ, навсякъде. Второто е по-евтино: грешката пада преди
 * коммита, а не пред човек, и не зависи от това кой браузър гледа.
 *
 * ═══ КАКВО СЕ БРОИ ═══
 *
...
const IZVAN_VRATATA = [
  ...
  [/\binsertAdjacentHTML\s*\(/, 'insertAdjacentHTML · вратата има `dopishi`'],
  [/\bdocument\.write\s*\(/, 'document.write'],
  [/(?<![.\w])eval\s*\(/, 'eval'],
  [/\bnew\s+Function\s*\(/, 'new Function'],
];
const VRATATA = 'app/reshetka/shablon.ts';

let vidyaniVrata = 0;
for (const f of kod) {
  if (izravni(relative(KOREN, f)) === VRATATA) continue;
  vidyaniVrata += 1;
  chist
    .get(f)
    .split('\n')
    .forEach((red, i) => {
      for (const [izraz, duma] of IZVAN_VRATATA) {
        if (izraz.test(red)) nahodka('9 · път до HTML извън вратата', f, i + 1, duma);
      }
    });
}
vidyal('9 · път до HTML извън вратата', vidyaniVrata);
...
const PRAGOVE = {
  ...
  '8 · дублирано': 1,
  /**
   * 8б е на ДВЕ, и двете с име (резен 6з · ADR-017):
   *
   *   · `formuli/izraz.ts` — нивата на приоритета в парсера (`sabirane` ↔
   *     `umnozhenie`). Те СА една форма, и точно това е правилното: сливането им
   *     значи един четец с флаг за нивото, а флаг вместо две функции е по-лошо;
   *   · `ogledalo/chettsi.ts` — регистърът на четците по тип събитие. Всеки
   *     четец е три реда с една и съща фигура; сливането им значи `switch`, а
   *     регистърът от четци беше избран НАРОЧНО вместо `switch` (ADR-003).
   *
   * Прагът е ЗАКОВАН на 2, не „поне 2": трета такава форма ще върне червено и
   * ще поиска обяснение, вместо да се прибави мълчаливо към тези.
   */
  '8б · дублирано по структура': 2,
  /**
