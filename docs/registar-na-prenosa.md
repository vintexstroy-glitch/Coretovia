...
**MasterBook е СВАЛЕН ОТ МРЕЖАТА на 06.09.2026, но ОСТАВА ИЗВОР.** Страницата му
на GitHub Pages е спряна (Source → None · Unpublish · средата `github-pages`
изтрита · хранилището архивирано), защото тя споделяше ПРОИЗХОД с Coretovia:
адресът е `<акаунт>.github.io` без пътя, тъй че съседната страница можеше да
отвори `indexedDB.open('coretovia')` и да прочете целия Журнал.

**Папката и тагът не се трият.** Днес 104 реда с присъда ПРЕНОС или ПРЕНАПИСВАНЕ
още сочат файл, който го няма в Coretovia, плюс 48 с присъда ЧАКА — общо **152
файла**, от които се хранят резен 7 (ИИ), резен 8 (кредити от ПДФ) и целият
продуктов слой. Изтрито хранилище значи 152 файла, писани наново.

Числата долу се БРОЯТ от таблицата (`tests/registar.test.ts`), не се преписват.

**ПРИСЪДАТА КАЗВА КАКВО ЩЕ СТАНЕ, НЕ КАКВО Е СТАНАЛО** (резен 6е · ADR-016).
Нито една от четирите не значи „вече е тук", тъй че „пренесено" и „ще се
пренесе" изглеждат еднакво — а за карта на дълга това е решаващата разлика.
Затова **ДОШЛОТО се БРОИ ОТ ДЪРВОТО**: `tests/registar.test.ts` проверява дали
целевият файл на всеки ред с ПРЕНОС или ПРЕНАПИСВАНЕ наистина съществува, и
държи числото с пин. Мръдне ли пинът, това се вижда в диф, вместо да се
промъкне.
